...
function drawScene() {
  ...
  var animate = function () {
    ...
    var squaresIndexes = [
      ...
      [-0.54, -0.62, 0.5, 0.2, 0.768, 0.643, 0.517], //house1
      [-0.54, -0.445, 0.45, 0.15, 0.36, 0.25, 0.2], //roof
      [-0.55, -0.65, 0.05, 0.14, 0, 0, 0], //door
      [-0.37, -0.57, 0.05, 0.05, 0, 0, 0], //window1
      [-0.71, -0.57, 0.05, 0.05, 0, 0, 0], //window2
    ...
    // skyg = 0.206
    // skyb = 0.235
    squares(0, 0.5, 2, 1, skyr, skyg, skyb);
    ...
    if(moony>=0.3)
    {
      for (let i = 0; i < 10; i++) 
    {
        const randomValuey = Math.random();
        const randomValuex = Math.random() * 2 - 1;
        circleProps(randomValuex,randomValuey, 0.003, 0.003, 1, 1, 1);   
    }
  }
    //this is moon
    if(moony>=0.82)
    ...
    circleProps(sunx, suny, 0.12, 0.12, 1, 1, 0);
    suny = suny - 0.0025;
    circleProps(moonx, moony, 0.12, 0.12, 1, 1, 1);
    moony = moony + 0.0025;
    ...
    // var birdIndex = [
    //   [bird10, bird11, 1.1, 55, -0.98, 55, 0.013, 0.68, 0.012, 0.03],
    ...
    // ];
    // bird10  = bird10 + 0.00002;
    // bird11 = bird11 + 0.00002;


    // for (let i = 0; i < 5; i++) {
    //   //right
    //   pushMatrix(matrixStack, mMatrix);
    //   mMatrix = mat4.rotate(mMatrix, degToRad(7), [0,0, 1.0]);
    ...
    //clouds and then sun
    cloud_rgb = cloud_rgb - 0.001
    circleProps(-0.9, 0.6, 0.17, 0.09,cloud_rgb,cloud_rgb,cloud_rgb);
    circleProps(-0.7, 0.57, 0.14, 0.07,cloud_rgb,cloud_rgb,cloud_rgb);
    circleProps(-0.56, 0.55, 0.12, 0.055,cloud_rgb,cloud_rgb,cloud_rgb);
